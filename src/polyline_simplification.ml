...
let erase_bits_between first last indexes () =
  for i = succ first to pred last do
    BitSet.unset indexes i
  done
...
let find_middle (type t) (module V : VECTOR with type t = t) sqr_epsilon first
    last a =
  if last < first + 2 then None
  else
    (* Square distance between a point and (first,last) segment *)
    let sqdist =
      let seg = V.sub a.(last) a.(first) in
      if is_zero ~eps:1e-6 (V.norm2 seg) then fun p ->
        V.(sub p a.(first) |> norm2)
      else
        let u = V.unit seg in
        fun p -> V.cross_norm2 u (V.sub a.(first) p)
    in
    Array.fold_lefti
  ...
  let rec simp = function
    ...
    | (first, last) :: tl -> (
      ...
      | None ->
          ...
          simp tl
      | Some middle -> simp ((first, middle) :: (middle, last) :: tl) )
  in
  simp [(0, pred sz)] ;
  indexes
